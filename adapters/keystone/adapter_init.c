...
static tee_status_t get_report(tee_buf_t* out){
    ...
    const char* p = getenv("KEYSTONE_REPORT_FILE");
    if(p && *p){
...
extern void mapping_add_extra_measurement(const uint8_t*, size_t);
...
static tee_status_t fill_platform_claims(const uint8_t* rpt, size_t n){
    mapping_reset();
    mapping_set_common("RISC-V-Keystone", "enclave", 0, 1);
    uint8_t meas[32] = {0};
    mapping_set_sw_measurement(meas, sizeof(meas));
    uint8_t sm_meas[32] = {0};
    mapping_add_extra_measurement(sm_meas, sizeof(sm_meas));
...
static tee_status_t ocall(uint32_t op, const void* in, size_t ilen, void* out, size_t* olen){
    ...
    size_t x = ilen < *olen ? ilen : *olen;
    if(in && x) memcpy(out, in, x);
    *olen = x;
    return TEE_OK;
}
...
static tee_class_t platform_id(void){ return TEE_CLASS_KEYSTONE; }
